...
# SPEED BOOST: Only run schema checks once per session!
    ...
    with admin_tab2:
        ...
        if not configs.empty:
            for _, r in configs.iterrows():
                occupied = conn.query("SELECT COUNT(*) as c FROM library_memberships WHERE config_id=:cid", params={"cid": r['id']}, ttl=0).iloc[0]["c"]
                ...
                with c2:
                    if st.button("Delete", key=f"del_code_{r['id']}", type="secondary", use_container_width=True):
                        with conn.session as s:
                            s.execute(text("DELETE FROM library_configurations WHERE id=:id"), {"id": r['id']})
                            s.commit()
                        st.rerun()
            ...
            for _, b in books.iterrows():
                ...
                with b2:
                    if st.button("Purge", key=f"purge_{b['id']}", type="secondary"):
                        with conn.session as s:
                            s.execute(text("DELETE FROM books WHERE id=:id"), {"id": b['id']})
                            s.commit()
                        st.rerun()
...
if st.session_state.library_config is None:
    ...
    if st.button("Verify & Open"):
        ...
        if not match.empty:
            cfg = match.iloc[0]
            members = conn.query("SELECT user_id FROM library_memberships WHERE config_id=:cid", params={"cid": cfg['id']}, ttl=0)["user_id"].tolist()
            ...
            if st.session_state.editing_book_id == b["id"]:
                ...
                with bc1:
                    if st.button("Save", key=f"sv_{b['id']}", use_container_width=True):
                        if e_title.strip():
                            with conn.session as s:
                                if e_file: s.execute(text("UPDATE books SET title=:t, category=:c, image_bytes=:img, image_name=:n WHERE id=:bid AND user_id=:uid"), {"t": e_title.strip(), "c": e_cat, "img": e_file.getvalue(), "n": e_file.name, "bid": b["id"], "uid": st.session_state.user_id})
                                else: s.execute(text("UPDATE books SET title=:t, category=:c WHERE id=:bid AND user_id=:uid"), {"t": e_title.strip(), "c": e_cat, "bid": b["id"], "uid": st.session_state.user_id})
                                s.commit()
